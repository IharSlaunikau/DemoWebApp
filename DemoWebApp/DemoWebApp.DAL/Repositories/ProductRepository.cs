...
public class ProductRepository : BaseRepository<Product>, IProductRepository
{
    ...
    public override async Task<Product> GetByIdAsync(int id) =>
        await Entities.AsNoTracking().Include(product => product.Category).FirstOrDefaultAsync(product => product.Id == id);

    public async Task<List<Product>> GetLimitedProducts(int maxAmount)
    {
        var products = maxAmount == ShowAllProducts
            ? await DbContext.Products.AsNoTracking().Include(product => product.Category).ToListAsync()
            : await DbContext.Products.AsNoTracking().Include(product => product.Category).Take(maxAmount).ToListAsync();
